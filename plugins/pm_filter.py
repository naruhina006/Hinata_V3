...
async def give_filter(client, message):
    ...
    if message.chat.id != SUPPORT_CHAT_ID:
        ...
        if total_results == 0:
            ...
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("𝐌𝐨𝐯𝐢𝐞 𝐑𝐞𝐪𝐮𝐞𝐬𝐭 𝐆𝐫𝐨𝐮𝐩 🌿", url=GRP_LNK)]]))
...
async def advantage_spoll_choker(bot, query):
    ...
    if files:
        ...
        contact_admin_button = InlineKeyboardMarkup(
            [[InlineKeyboardButton("Cʟɪᴄᴋ ʜᴇʀᴇ ᴛᴏ ᴊᴏɪɴ ʀᴇǫᴜᴇsᴛ ɢʀᴏᴜᴘ", url=OWNER_LNK)]])
        ...
        if len(message.text) < 100:
            ...
            if not files:
                if settings["spell_check"]:
                    ai_sts = await m.edit('<b>ᴡᴀɪᴛ, ᴀɪ ᴄᴏʀʀᴇᴄᴛɪɴɢ ʏᴏᴜʀ ꜱᴘᴇʟʟɪɴɢ 🌿</b>')
                    is_misspelled = await ai_spell_check(chat_id = message.chat.id,wrong_name=search)
                    if is_misspelled:
                        await ai_sts.edit(f'<b>ᴀɪ ꜱᴜɢɢᴇꜱᴛᴇᴅ 🌿<code> {is_misspelled}</code> \nꜱᴏ ɪ ᴀᴍ ꜱᴇᴀʀᴄʜɪɴɢ ᴛʜɪꜱ 🌿<code>{is_misspelled}</code></b>')
